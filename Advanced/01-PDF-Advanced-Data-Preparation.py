...
# MAGIC
# MAGIC CREATE TABLE IF NOT EXISTS main.rag_chatbot_callum_elder.armhub_pdf_documentation (
...
# MAGIC   cohere_embedding ARRAY <FLOAT>
# MAGIC ) TBLPROPERTIES (delta.enableChangeDataFeed = true); 
# MAGIC
# MAGIC DESCRIBE DETAIL main.rag_chatbot_callum_elder.armhub_pdf_documentation
...
# MAGIC %md
# MAGIC ###Cohere Embedding Endpoint

# COMMAND ----------

from mlflow.deployments import get_deploy_client

# create endpoint for cohere model
      ...
      "served_entities": [
        {
          ...
          "external_model": {
            ...
            "task": "llm/v1/embeddings",
              "cohere_config": {
            "cohere_api_key": "{{secrets/callum-cohere/cohere_api_key}}"
            }
          },
        }
      ],
...
@pandas_udf("array<float>")
def get_cohere_embedding(contents: pd.Series) -> pd.Series:
    import mlflow.deployments
    ...
    def get_embeddings(batch):
        try:
            response = deploy_client.predict(endpoint="embed-english-v3.0", inputs={"input": batch})
            return [e['embedding'] for e in response.data]
        except Exception as e:
            print(f"Error while getting embeddings: {str(e)}")
            return [None] * len(batch)
...
volume_location = "/Volumes/main/rag_chatbot_callum_elder/armhub_sample"

# for file in os.listdir(volume_location):    
file = os.listdir(volume_location)[0]
try:
    file_path = os.path.join(volume_location, file)
    print("Volume Location:")
    print(volume_location)
    print("---")
    
    with open(file_path, 'rb') as file:
        print("File exists in the volume location.")
        pdf_content = file.read()
        pdf_content_encoded = base64.b64encode(pdf_content).decode('utf-8')
        basename = file_path.rsplit('.', 1)[0]
        pdf_df = spark.createDataFrame([(volume_location, basename, pdf_content_encoded)], ["url", "basename", "content"])
        ...
        embedded_df = exploded_df.select("url", "basename", "content", get_bge_embedding("content").alias("bge_embedding"), get_ada_embedding("content").alias("ada_embedding"), get_cohere_embedding("content").alias("cohere_embedding"))
        
        delta_table_path = "main.rag_chatbot_callum_elder.armhub_pdf_documentation"
        embedded_df.write.format("delta").option("mergeSchema", "true").mode("append").saveAsTable(delta_table_path)
        print("Chunking, embedding, and saving to Delta table completed.")

except FileNotFoundError:
    print("File does not exist in the volume location.")
    print("---")
...
# storage_account_url = "https://stucarmhubprotoeastus2.queue.core.windows.net"
# queue_name = "volume2delta"
# managed_identity_client_id = '5b23136d-fbf1-4fcb-aa29-05b788298dca'

# credential = ManagedIdentityCredential(client_id=managed_identity_client_id)
# queue_client = QueueClient(storage_account_url, queue_name=queue_name, credential=credential)
...
#             blob_url = message_content["data"]["url"]
#             file_path = blob_url.split("/")[-1]
#             volume_location = f"/Volumes/main/rag_chatbot_callum_elder/armhub_sample/{file_path}"
#             print("Volume Location:")
#             print(volume_location)
#             print("---")
            
#             with open(volume_location, 'rb') as file:
#                 print("File exists in the volume location.")
#                 pdf_content = file.read()
#                 basename = file_path.rsplit('.', 1)[0]
#                 pdf_df = spark.createDataFrame([(volume_location, basename, pdf_content)], ["url", "basename", "content"])
                
#                 chunked_df = pdf_df.select("url", "basename", read_as_chunk("content").alias("chunks"))
#                 exploded_df = chunked_df.select("url", "basename", explode("chunks").alias("content"))
#                 bge_embedded_df = exploded_df.select("url", "basename", "content", get_bge_embedding("content").alias("bge_embedding"))
#                 ada_embedded_df = exploded_df.select("url", "basename", "content", get_ada_embedding("content").alias("ada_embedding"))
#                 cohere_embedded_df = exploded_df.select("url", "basename", "content", get_cohere_embedding("content").alias("cohere_embedding"))
                
#                 delta_table_path = "main.rag_chatbot_callum_elder.armhub_pdf_documentation"
#                 bge_embedded_df.write.format("delta").option("mergeSchema", "true").mode("append").saveAsTable(delta_table_path)
#                 ada_embedded_df.write.format("delta").option("mergeSchema", "true").mode("append").saveAsTable(delta_table_path)
#                 cohere_embedded_df.write.format("delta").option("mergeSchema", "true").mode("append").saveAsTable(delta_table_path)
#                 print("Chunking, embedding, and saving to Delta table completed.")
...
#     # print(f"Processed {len(messages)} messages. Waiting for {delay_between_batches} seconds before retrieving the next batch.")
